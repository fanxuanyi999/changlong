(function(){
 ...
 const pathFor=(p,state='')=>`<path class="${state}" d="${line(p)}"/>`;
 ...
 class DayTwoMap extends HTMLElement{
  connectedCallback(){
   ...
   this.innerHTML=`<section class="pm-frame" aria-label="Day2园内互动地图"><header class="pm-heading"><div><h3>今天，跟着这条线走</h3><p>长隆野生动物世界 · 9月25日 · 12站亲子慢游</p></div><span>15:30 收尾 · 20:00 大马戏</span></header>
    ...
    <div class="pm-stage"><div class="pm-surface"><svg class="pm-map" data-coordinate-system="normalized-1200x900" viewBox="0 0 1200 900" role="group" aria-label="园区互动路线示意图，可拖拽和双指缩放"><g class="pm-illustration-base" data-map-layer="illustration" aria-hidden="true" pointer-events="none"><image class="pm-artwork" href="assets/park-painted-v3.webp" width="1200" height="900"/></g><g class="pm-navigation-overlay" data-map-layer="navigation" pointer-events="none"><g class="pm-paths"></g><g class="pm-you"></g></g><g class="pm-interactive-poi" data-map-layer="poi"><g class="pm-nodes"></g><g class="pm-facilities"></g></g></svg></div><div class="pm-map-key"><i></i><span>① → ⑫ · 单向游览顺序</span></div><div class="pm-zoom"><button type="button" data-action="zoom-in" aria-label="放大地图">＋</button><button type="button" data-action="zoom-out" aria-label="缩小地图">−</button><button type="button" data-action="fit" aria-label="显示全图">全图</button></div><div class="pm-hint">区域示意 · 按现场路牌行走 · 可拖拽 / 双指缩放</div><section class="pm-sheet" data-map-ui="poi-details" aria-label="点位详情" hidden></section></div>
    ...
    <div class="pm-controls"><label>找一站 <select aria-label="选择地图节点"><option value="">点击地图或选择节点</option>${D.nodes.map(n=>`<option value="${n.id}">${n.number} · ${n.name}</option>`).join('')}</select></label><button type="button" data-action="minimal">全部关闭</button><button type="button" data-action="offline">保存到离线</button></div><p class="pm-message" role="status" aria-live="polite"></p></section>
    <details class="pm-source"><summary>地图、定位与离线说明</summary><p>手绘底图由 AI 辅助创作，根据<a href="${D.source}" target="_blank" rel="noopener">官网园区导览图</a>转绘为空间关系示意，不是比例地图。设施标记为所在区域，未核验精确入口、距离与实时开放状态；休息点是建议停留区域。<a href="${D.mapSource}" target="_blank" rel="noopener">对照官网原图</a>。整理于2026-09-22。</p><p>浏览器定位仅在点击后申请，坐标仅在本机内存中使用。插画无可靠经纬度配准，获取GPS后需手动校准到节点；蓝点表示你确认的位置，不自动跟随。手动位置最多保留6小时，移动后请更新。路线是游览顺序，实际步道以路牌为准。</p><p>首次联网打开并显示“离线已就绪”后，可离线使用地图、节点详情和文字攻略。外部地图、官网与实时定位不保证离线可用。浏览器清理站点数据会移除缓存。原“保存离线版”仍是文字攻略文件；互动地图请使用此处离线缓存。</p></details>`;
  ...
  render(){
   ...
   this.renderPaths();
   this.querySelector('.pm-nodes').innerHTML=D.nodes.map(n=>{const left=n.x>980,signWidth=Math.max(98,n.name.length*19+22),state=c?(n.id===c.id?'is-current':n.number<c.number?'is-complete':n.id===next?.id?'is-next':''):'';return `<g class="pm-node ${this.selected===n.id?'selected':''} ${state} ${this.target===n.id?'is-target':''}" data-node="${n.id}" role="button" tabindex="0" aria-label="${n.number} ${html(n.name)} ${n.time}" transform="translate(${n.x},${n.y})"><circle r="28" fill="transparent"/><circle class="pm-node-halo" r="27"/><circle class="pm-pin" r="20"/><rect class="pm-sign" style="--pm-sign-short-width:${Math.max(98,n.name.split(' / ')[0].length*19+22)}px" x="${left?-24-signWidth:24}" y="-24" width="${signWidth}" height="51" rx="4"/><text class="pm-number">${n.number}</text><text class="pm-label" x="${left?-30:30}" y="-3" text-anchor="${left?'end':'start'}">${html(n.name.split(' / ')[0])}<tspan class="pm-label-extra">${n.name.includes(' / ')?html(' / '+n.name.split(' / ').slice(1).join(' / ')):''}</tspan></text><text class="pm-time" x="${left?-30:30}" y="18" text-anchor="${left?'end':'start'}">${n.time}</text></g>`}).join('');
   this.querySelector('.pm-facilities').innerHTML=D.facilities.filter(f=>f.type===this.layer).map(f=>`<g class="pm-facility ${this.target===f.id?'is-target':''}" data-facility="${f.id}" data-type="${f.type}" role="button" tabindex="0" aria-label="${html(f.name)}，区域示意" transform="translate(${f.x},${f.y})"><circle r="24"/><text>${symbol[f.type]}</text></g>`).join('');
  ...
  renderPaths(){
   ...
   const currentIndex=D.nodes.findIndex(n=>n.id===this.current);
   const main=D.segments.map((s,i)=>pathFor(s.points,i<currentIndex?'pm-route-complete':'pm-route-ahead')).join('');
  ...
  click(e){
   ...
   if(a==='fit')this.fit();
   if(a==='minimal')this.querySelector('[data-layer=route]').click();
   ...
   if(a==='route'){this.closeSheet(false);this.target=null;this.querySelector('.pm-guidance').hidden=true;this.render();this.fit();}
  ...
  applyView(){const v=this.view;v.x=Math.max(-1200,Math.min(1800,v.x));v.y=Math.max(-900,Math.min(1350,v.y));this.svg.setAttribute('viewBox',`${v.x} ${v.y} ${v.w} ${v.h}`);const scale=this.svg.getBoundingClientRect().width/v.w;this.dataset.mapDetail=scale<.46?'overview':scale<.85?'area':'detail';}
  ...
  setupGestures(){
   // Keep native page gestures outside the SVG untouched. The HTML surface also
   // establishes touch-action before pointer dispatch (including Safari/WebViews).
   const stopNative=e=>{if(e.cancelable)e.preventDefault();};
   const stopPinch=e=>{if(e.touches.length>1)stopNative(e);};
   this.svg.addEventListener('touchstart',stopPinch,{passive:false});
   this.svg.addEventListener('touchmove',stopPinch,{passive:false});
   for(const type of ['gesturestart','gesturechange','gestureend'])this.svg.addEventListener(type,stopNative,{passive:false});
   this.svg.addEventListener('dblclick',e=>{if(e.target.closest('[data-node],[data-facility]'))return;e.preventDefault();this.zoom(.72,this.mapPoint(e.clientX,e.clientY));});
